...
/// Encrypts data using simplified DES with specified encoding
/// 
...
pub fn encrypt(data: &str, key: &str, encoding: &str) -> Result<String> {
    ...
    for chunk in data_bytes.chunks(8) {
        let encrypted_block = des_encrypt_block(chunk, key_bytes)?;
...
/// Decrypts data using simplified DES with specified encoding
/// 
/// # Key Requirements
...
pub fn decrypt(data: &str, key: &str, encoding: &str) -> Result<String> {
    ...
    for chunk in encrypted_bytes.chunks(8) {
        ...
        let decrypted_block = des_decrypt_block(chunk, key_bytes)?;
